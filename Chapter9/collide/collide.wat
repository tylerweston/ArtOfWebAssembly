(module
    ...
    (func $clear_canvas
        ...
        i32.const 4
        i32.mul     ;; 4 bytes/pixel

        local.set $pixel_bytes

        (loop $pixel_loop
            (i32.store (local.get $i) (i32.const 0xff_00_00_00))    ;; erase pixel
            (i32.add (local.get $i) (i32.const 4))
            local.set $i        ;; i += 4 bytes
            ;; if $i < $pixel_bytes
    ...
    (func $abs
        ...
        if
            ...
            ;; note, using 0 - value is faster than using 2s complement!
        end
        local.get $value    ;; return original value
    )

    ;; convert 2d to 1d coords and set a pixel
    (func $set_pixel
        (param $x   i32)
        (param $y   i32)
        (param $c   i32)
        ...
        local.get $x
        i32.add     ;; $x + $y * $cnvs_size

        i32.const 4
        i32.mul     ;; each pixel is 4 bytes

        local.get $c
        i32.store   ;; store color at location calculated
    )
    ...
    ;; get the attribute of an object in linear memoyr
    (func $get_obj_attr
        (param $obj_number  i32)
        (param $attr_offset i32)
        (result i32)
        ...
        i32.load    ;; load pointer
    )

    ;; move and detect collision between all objects
    (func $main (export "main")
        ...
        (loop $outer_loop (block $outer_break
            ...
            local.set $i_hit    ;; set $i_hit to 0 = false
            ...
            (loop $inner_loop (block $inner_break
                ...
                i32.ge_u
                if
                    br $inner_break ;; if $j >= $obj_cnt, break from inner loop
                end
                ...
                if  ;; if #xdist >= $obj_size, no collision
                    local.get $j
                    i32.const 1
                    i32.add
                    local.set $j
            ...
            i32.lt_u
            if
                br $outer_loop  ;; if $i < $obj_cnt, jump to top of outer loop
            end
        ))  ;; end of outer loop
    )       ;; end of function
)           ;; end of module